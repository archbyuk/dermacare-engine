"""
Product_Event 테이블용 Excel 파서
이벤트 상품 데이터를 Excel에서 읽어 DB에 삽입 (날짜 변환 포함)
"""
...
class ProductEventParser(AbstractParser):
    """이벤트 상품 테이블 파서 (날짜 변환 포함)"""
    
    def __init__(self, db_session: Session):
        super().__init__(db_session, "Product_Event")
    
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        """
        Product_Event 테이블 데이터 검증
        """
        errors = []
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """데이터 정리"""
        print(f"DEBUG: Product_Event clean_data 시작 - DataFrame 컬럼: {df.columns.tolist()}")
        ...
        for col in int_columns:
            if col in df.columns:
                ...
                print(f"DEBUG: 정수 컬럼 없음 - {col}")
        
        # 실수 컬럼들 정리
        float_columns = ['Discount_Rate', 'Margin_Rate']
        for col in float_columns:
            if col in df.columns:
                print(f"DEBUG: 실수 컬럼 처리 중 - {col}")
                df[col] = pd.to_numeric(df[col], errors='coerce')
                # nan 값을 None으로 변환
                df[col] = df[col].replace([pd.NA, pd.NaT, float('nan'), 'nan'], None)
                df[col] = df[col].where(df[col].notna(), None)
            else:
                print(f"DEBUG: 실수 컬럼 없음 - {col}")
        
        # 날짜 컬럼들을 MySQL DATE 타입으로 변환
        date_columns = ['Event_Start_Date', 'Event_End_Date']
        ...
        print(f"DEBUG: Product_Event clean_data 완료 - 최종 컬럼: {df.columns.tolist()}")
        return df
    
    def insert_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        Product_Event 테이블에 데이터 삽입
        """
        try:
            total_rows = len(df)
            inserted_count = 0
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    product_event = ProductEvent(
                        ...
                        Event_Info_ID=row.get('Event_Info_ID'),
                    ...
                    # DB에 추가 (REPLACE 방식)
                    existing = self.db.query(ProductEvent).filter(
                        ProductEvent.ID == row.get('ID')
                    ).first()
