"""
Info_Event 테이블용 Excel 파서
이벤트 정보 데이터를 Excel에서 읽어 DB에 삽입
"""
...
from db.models.info import InfoEvent


class InfoEventParser(AbstractParser):
    """이벤트 정보 테이블 파서"""
    
    def __init__(self, db_session: Session):
        super().__init__(db_session, "Info_Event")
    
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        """
        Info_Event 테이블 데이터 검증 (ID가 NULL인 행은 제외)
        """
        errors = []
        ...
        # 숫자 컬럼 검증
        numeric_columns = ['ID', 'Release', 'Event_ID']
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Info_Event 데이터 정리 (ID가 NULL인 행은 제외)
        """
        # ID가 NULL이 아닌 행만 필터링
        df = df[df['ID'].notna()]
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # DB에 추가 (REPLACE 방식)
                    existing = self.db.query(InfoEvent).filter(
                        InfoEvent.ID == row.get('ID')
                    ).first()
