"""
    [ Procedure_Element Parser ]
        단일 시술 DataFrame을 읽어 DB에 삽입하는 파서
...
class ProcedureElementParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        # 숫자 컬럼 검증
        numeric_columns = ['ID', 'Release', 'Plan_Count', 'Plan_Interval', 'Consum_1_ID', 'Consum_1_Count', 'Procedure_Cost', 'Price']
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(ProcedureElement).delete()
                ...
                self.db.bulk_insert_mappings(ProcedureElement, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Procedure_Element 테이블 삽입 완료")
