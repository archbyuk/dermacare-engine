"""
    [ Membership Parser ]
        맴버십 상품 DataFrame을 읽어 DB에 삽입하는 파서
...
class MembershipParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        # 숫자 컬럼 검증
        numeric_columns = [
            'ID', 'Release', 'Membership_Info_ID', 'Payment_Amount', 'Bonus_Point', 'Credit',
            'Discount_Rate', 'Element_ID', 'Bundle_ID', 'Custom_ID', 'Sequence_ID',
            'Validity_Period'
        ]
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                ...
                self.db.bulk_insert_mappings(Membership, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Membership 테이블 삽입 완료")
