"""
    [ Info_Event Parser ]
       이벤트 상품 정보 DataFrame을 읽어 DB에 삽입하는 파서
"""
...
class InfoEventParser(AbstractUtils):
    """
        params:
    ...
    # Info_Event 테이블에 데이터 삽입 (DB insert)
    async def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.db.begin():
                # 기존 데이터 전체 삭제 (비동기)
                await self.db.execute(delete(InfoEvent))
                ...
                # 배치 삽입 (비동기)
                if insert_list:
                    await self.db.execute(insert(InfoEvent), insert_list)
