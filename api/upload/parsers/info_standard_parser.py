"""
    [ Info_Standard Parser ]
        스탠다드 상품 정보 DataFrame을 읽어 DB에 삽입하는 파서
...
class InfoStandardParser(AbstractUtils):
    """
        params:
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(InfoStandard).delete()
                ...
                self.db.bulk_insert_mappings(InfoStandard, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Info_Standard 테이블 삽입 완료")
