"""
    [ Global Parser ]
        전역 설정 데이터를 DataFrame을 읽어 DB에 삽입하는 파서
"""
...
class GlobalParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        print(f"[DEBUG] Global 검증 숫자 컬럼 확인: {existing_columns}")
        ...
        print("[Debug] Global 검증 결과: ", errors)
        return len(errors) == 0, errors

    # Global 테이블에 데이터 삽입 (DB insert)
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(Global).delete()

                # 삽입할 데이터 리스트: 데이터프레임을 딕셔너리 리스트로 변환
                # ex) 
                # [
                #    {'ID': 1, 'Doc_Price_Minute': 10000, 'Aesthetician_Price_Minute': 10000}, 
                #    {'ID': 2, 'Doc_Price_Minute': 10000, 'Aesthetician_Price_Minute': 10000}
                # ]
                ...
                self.db.bulk_insert_mappings(Global, insert_list)

            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Global 테이블 삽입 완료")
