"""
    [ Product_Event Parser ]
        이벤트 상품 DataFrame을 읽어 DB에 삽입하는 파서
...
class ProductEventParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        # 숫자 컬럼 검증
        numeric_columns = [
            'ID', 'Release', 'Element_ID', 'Bundle_ID', 'Sequence_ID',
            'Standard_Info_ID', 'Procedure_Cost', 'Original_Price', 
            'Sell_Price', 'Margin', 'Validity_Period', 'VAT', 'Discount_Rate',
            'Margin_Rate'
        ]
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(ProductEvent).delete()
                ...
                self.db.bulk_insert_mappings(ProductEvent, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Product_Event 테이블 삽입 완료")
