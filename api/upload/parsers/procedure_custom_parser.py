"""
    [ Procedure_Custom Parser ]
        커스텀 시술 DataFrame을 읽어 DB에 삽입하는 파서
...
class ProcedureCustomParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        # 필수 컬럼 리스트
        required_columns = ['GroupID', 'ID', 'Release', 'Element_ID']
        ...
        # 숫자 컬럼 검증
        numeric_columns = ['GroupID', 'ID', 'Release', 'Element_ID', 'Custom_Count', 'Element_Limit', 'Element_Cost', 'Price_Ratio']
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(ProcedureCustom).delete()
                ...
                self.db.bulk_insert_mappings(ProcedureCustom, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Procedure_Custom 테이블 삽입 완료")
