"""
    [ Procedure_Bundle Parser ]
        번들 시술 DataFrame을 읽어 DB에 삽입하는 파서
...
class ProcedureBundleParser(AbstractUtils):
    """
        params:
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(ProcedureBundle).delete()
                ...
                self.db.bulk_insert_mappings(ProcedureBundle, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Procedure_Bundle 테이블 삽입 완료")
