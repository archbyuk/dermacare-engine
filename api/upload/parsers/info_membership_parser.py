...
class InfoMembershipParser(AbstractUtils):
    """
        params:
    ...
    def validate_data(self, used_df: pd.DataFrame, filename: str) -> Tuple[bool, List[str]]:
        ...
        # 숫자 컬럼 검증
        numeric_columns = ['ID', 'Release', 'Membership_ID']
    ...
    def insert_data(self, used_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.begin():
                # 기존 데이터 전체 삭제
                self.db.query(InfoMembership).delete()
                ...
                self.db.bulk_insert_mappings(InfoMembership, insert_list)
                
            # with self.db.begin(): 트랜잭션 커밋 자동 적용 및 롤백 자동 적용

            print("[DEBUG] Info_Membership 테이블 삽입 완료")
